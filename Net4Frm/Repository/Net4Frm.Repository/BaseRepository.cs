...
namespace Net4Frm.Repository
{
    public abstract class BaseRepository<T> : IBaseRepository<T>, IUnitOfWorkRepository where T : ModelBase
    {
        ...
        public int CheckField(string _tableName, string _columnName, string _value)
        {
            string sql = "";
            string whereStr = string.Empty;
            if (_tableName != null && _columnName != null && _value != null)
            {
                sql = string.Format("select count(*) from [{0}] where {1} = '{2}' and is_deleted='0'", _tableName, _columnName, _value);
            }
        ...
        private IEnumerable<TY> DapperSql<TY>(string sql)
        {
            ...
            using (
                IDbConnection dbConnection =
                    new SqlConnection(Configurator.DbConnectionString))
            {
                try
                {
                    dbConnection.Open();
